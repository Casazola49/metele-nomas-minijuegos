...
- [ ] Verify: `npm test`, `npm run lint`, `npx tsc --noEmit`, `npm run build` pass; `/games/pelimojis` still renders yellow GameShell. <!-- sdd-owner: implementation -->

## Slice 2: Hero + GameGrid (PR 2)

- [ ] Rewrite `components/home/Hero.tsx`: remove action words, parallax transforms, "No te lo pierdas" section, and dividers; add 3 `landing-blob-float-*` blobs, `landing-dot-grid`, top/bottom `landing-light-line`, `landing-logo-ring` around `/images/metele-logo.png`, `landing-gradient-text` "METELE NOMÁS", tag "PODCAST · STREAMING · MINIJUEGOS", `<a href="#games">` JUGAR CTA. <!-- sdd-owner: implementation -->
...
- [ ] Run bounded review over the chained PRs and re-confirm `/games/pelimojis` visual baseline unchanged after all slices. <!-- sdd-owner: parent -->
